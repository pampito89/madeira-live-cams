...
export default function PlannerRouteFinish({ locale, name, airport, arrival, travel, calculated, error }: Props) {
  return <>
    <section aria-label={locale === 'uk' ? 'Кінцева точка маршруту' : 'Route destination'} className="mt-3 hidden rounded-2xl border border-ocean/30 bg-ocean/5 p-4 sm:block">
    ...
    {error && <p role="alert" className="mt-3 text-sm font-semibold text-red-700 sm:hidden">{error}</p>}
  </>;
}
