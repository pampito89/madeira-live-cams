...
export default function CustomDayPoint({ id, value, onChange, onAdd, isAdded, locale, resolution }: Props) {
  const [clipboardError, setClipboardError] = useState(false);
  const ukrainian = locale === 'uk';
  const addLabel = ukrainian ? 'Додати точку до маршруту дня' : 'Add point to day route';
  return <div className="mt-4">
    ...
    <div className="mt-1.5 flex items-stretch gap-2">
      ...
      <button type="button" onClick={onAdd} disabled={!resolution.point || resolution.loading}
        aria-label={addLabel} title={addLabel}
        className="flex min-h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-ocean text-xl font-bold text-white transition hover:bg-forest focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-40">
        +
      </button>
    </div>
    <p id={`${id}-status`} role="status" className={`mt-2 break-words text-xs ${resolution.error ? 'text-red-600' : 'text-slate-500'}`}>
      {resolution.loading ? (ukrainian ? 'Перевіряємо точку…' : 'Looking up the point…')
        ...
        : resolution.point ? `${isAdded ? (ukrainian ? 'Точку додано до маршруту дня' : 'Point added to the day route') : (ukrainian ? 'Точку знайдено — натисніть +, щоб додати її до маршруту дня' : 'Point found — press + to add it to the day route')}: ${resolution.point.name} (${resolution.point.latitude}, ${resolution.point.longitude})`
        : (ukrainian ? 'Вставте коротке або повне посилання Google Maps чи координати, а потім натисніть +.' : 'Paste a short or full Google Maps link or coordinates, then press +.')}
    </p>
