...
const copy = {
  en: {
    ...
    steps: [
      '1. From the parking area, walk uphill to Levada Nova. Allow roughly 15–20 minutes and around 100 m of ascent for this approach; these are estimates, not measured route figures.',
      '2. Follow Levada Nova towards the tunnel. The tunnel is approximately 200 m long. Bring a headlamp or use your phone torch, walk carefully and watch your head: the entrance is low.',
      '3. After the tunnel, continue past the two waterfalls. The path can be wet and slippery. Keep going until you see the connecting steps down to the lower levada; do not miss this descent.',
      '4. Take the steps down to Levada do Moinho. At the bottom turn left and stay on the lower levada without taking side paths. Follow it all the way back to the parking area.',
    ],
  ...
  uk: {
    ...
    steps: [
      '1. Від парковки підніміться пішки до Levada Nova. Орієнтовно закладіть 15–20 хвилин і близько 100 м набору висоти; це оцінка, а не виміряні параметри маршруту.',
      '2. Ідіть Levada Nova до тунелю. Його довжина — приблизно 200 м. Візьміть налобний ліхтар або ввімкніть ліхтарик телефона. На вході стеля низька: бережіть голову й дивіться під ноги.',
      '3. Після тунелю пройдіть повз два водоспади. Стежка тут може бути мокрою та слизькою. Продовжуйте до сходів, що з’єднують верхню й нижню левади: важливо не пропустити цей спуск.',
      '4. Спустіться сходами на Levada do Moinho. Унизу поверніть ліворуч і йдіть нижньою левадою без відгалужень аж до парковки.',
    ],
...
export default function LevadaNovaRoute({ locale }: Props) {
  ...
  return (
    <section className="mt-8" aria-labelledby="levada-route-heading">
      ...
      <ol className="mt-5 space-y-3 text-slate-700">{text.steps.map((step) => <li key={step} className="leading-7">{step}</li>)}</ol>
