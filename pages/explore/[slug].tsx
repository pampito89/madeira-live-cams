...
import { getLocalizedLocation, getLocationBySlug, locations, type Location, } from "../../data/plannerLocations";
...
function wazeNavigationUrl(destination: string, usesCoordinates: boolean) {
    if (usesCoordinates) {
        return `https://www.waze.com/ul?ll=${encodeURIComponent(destination)}&navigate=yes`;
    }
    return `https://www.waze.com/ul?q=${encodeURIComponent(destination)}&navigate=yes`;
}
function StandardLocationPage({ location }: LocationPageProps) {
    ...
    const navigationDestination = navigationDestinations[displayLocation.slug];
    ...
    const handleShare = async () => {
        ...
        try {
            ...
            try {
                ...
                window.prompt(locale === 'uk'
                    ? 'Скопіюйте це посилання:'
                    : 'Copy this link:', pageUrl);
            }
        }
    };
    return (<Layout>
      <Head>
        ...
        <meta property="og:title" content={`${displayLocation.name} | ${messages.location.ogTitleSuffix}`}/>
        ...
        <meta name="twitter:title" content={`${displayLocation.name} | Madeira Live Cams`}/>
      ...
      <main className="page-shell">
        <button type="button" onClick={handleBack} className="inline-flex items-center gap-2 rounded-full border border-ocean/30 bg-white px-3 py-1.5 text-sm font-semibold text-ocean shadow-sm transition hover:-translate-x-0.5 hover:border-ocean hover:bg-ocean hover:text-white focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2">
        ...
        <article className="mx-auto mt-5 max-w-3xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="relative aspect-[16/9] bg-slate-100">
            <Image src={displayLocation.image} alt={displayLocation.imageAlt} fill priority className="object-cover" sizes="(max-width: 768px) 100vw, 768px"/>
          </div>
          ...
          <div className="p-5 sm:p-8">
            ...
            <div className="mt-2">
              ...
              <div className="mt-3 flex flex-wrap gap-2"><Link href={`/trip-plan?addLocation=${encodeURIComponent(displayLocation.slug)}`} className="inline-flex h-10 items-center justify-center whitespace-nowrap rounded-lg bg-ocean px-3 text-sm font-semibold text-white transition hover:bg-forest focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ocean focus-visible:ring-offset-2">+ {locale === "uk" ? "До плану" : "Add to plan"}</Link>
                <div className="relative">
                  ...
                  {isNavigationOpen && (<div className="absolute left-0 top-12 z-20 w-48 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
                      <a href={googleMapsDirectionsUrl(navigationQuery)} target="_blank" rel="noopener noreferrer" onClick={() => setIsNavigationOpen(false)} className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-navy transition hover:bg-panel hover:text-ocean">
                        <span aria-hidden="true">🗺️</span>
                        Google Maps
                      </a>

                      <a href={wazeNavigationUrl(navigationQuery, Boolean(navigationDestination))} target="_blank" rel="noopener noreferrer" onClick={() => setIsNavigationOpen(false)} className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-navy transition hover:bg-panel hover:text-ocean">
                        <span aria-hidden="true">🚗</span>
                        Waze
                      </a>

                      <button type="button" onClick={() => setIsNavigationOpen(false)} aria-label={locationText.closeRoute} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-medium text-slate-500 transition hover:bg-slate-100 hover:text-navy">
                        <span aria-hidden="true">×</span>
                        {locationText.close}
                      </button>
                    </div>)}
                ...
                <button type="button" onClick={handleShare} aria-label={locationText.share} title={locationText.share} className="inline-flex h-10 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-ocean bg-white px-3 text-sm font-semibold text-ocean transition hover:bg-ocean hover:text-white focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2">
                  <span aria-hidden="true">↗</span>
            ...
            <a href={googleMapsLocationUrl(displayLocation.mapQuery)} target="_blank" rel="noopener noreferrer" className="mt-8 inline-flex w-full items-center justify-center rounded-lg border border-ocean bg-white px-4 py-3 text-sm font-semibold text-ocean transition hover:bg-ocean hover:text-white focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2 sm:w-auto">
              {locationText.map}
            </a>
          </div>
        </article>
...
export const getStaticPaths: GetStaticPaths = async () => {
    const locales = ['en', 'uk'];
    return {
        paths: locations.flatMap((location) => locales.map((locale) => ({
