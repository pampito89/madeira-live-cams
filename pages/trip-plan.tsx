...
<label className="flex flex-col gap-1.5 text-sm font-semibold text-navy">{text.startVilla}<select value={startPoint} onChange={(event) => { const value = event.target.value; setStartPoint(value); setDepartureTime(value === airportStartPoint.slug ? '17:00' : '09:00'); if (value !== airportStartPoint.slug && value !== customStartPoint) setVilla(value); }} className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-navy focus:border-ocean focus:outline-none focus:ring-2 focus:ring-ocean/20"><option value={airportStartPoint.slug}>{locale === 'uk' ? 'Аеропорт Мадейри' : 'Madeira Airport'}</option><option value={customStartPoint}>{locale === 'uk' ? 'Довільна точка' : 'Custom point'}</option>{villas.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label>
...
{isCustomStart && <div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="flex flex-col gap-1.5 text-sm font-semibold text-navy">{locale === 'uk' ? 'Посилання Google Maps або координати' : 'Google Maps link or coordinates'}<span className="relative block"><input value={customPointInput} onChange={(event) => setCustomPointInput(event.target.value)} placeholder="https://maps.google.com/?q=32.6919,-16.7745" className="min-h-11 w-full rounded-lg border border-slate-300 bg-white py-2 pl-3 pr-11 text-sm font-medium text-navy focus:border-ocean focus:outline-none focus:ring-2 focus:ring-ocean/20" /><button type="button" onClick={async () => { try { setCustomPointInput(await navigator.clipboard.readText()); } catch { } }} aria-label={locale === 'uk' ? 'Вставити з буфера' : 'Paste from clipboard'} title={locale === 'uk' ? 'Вставити' : 'Paste'} className="absolute inset-y-1 right-1 flex w-9 items-center justify-center rounded-md text-base text-ocean hover:bg-ocean/10">📋</button></span></label><label className="flex flex-col gap-1.5 text-sm font-semibold text-navy">{text.endPoint}<select value={endVilla} onChange={(event) => setEndVilla(event.target.value)} className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-navy focus:border-ocean focus:outline-none focus:ring-2 focus:ring-ocean/20"><option value="">{text.chooseEndVilla}</option>{villas.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label></div>}
{isAirportStart && <div className="mt-4 max-w-sm"><label className="flex flex-col gap-1.5 text-sm font-semibold text-navy">{text.endPoint}<select value={endVilla} onChange={(event) => setEndVilla(event.target.value)} className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-navy focus:border-ocean focus:outline-none focus:ring-2 focus:ring-ocean/20"><option value="">{text.chooseEndVilla}</option>{villas.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label></div>}
</div>
