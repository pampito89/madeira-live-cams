...
export default function TripPlanPage() {
  ...
  useEffect(() => {
    ...
    const pointForStop = (stop: PlanStop): [number, number] | null => {
      if (stop.type === 'villa') return [selectedVilla.latitude, selectedVilla.longitude];
      if (stop.type === 'location' && stop.slug) return locationCoordinates[stop.slug] ?? null;
      return null;
    };
    const calculate = async () => {
      setRouteStatus('loading');
      let previous: [number, number] | null = [selectedVilla.latitude, selectedVilla.longitude];
      ...
      for (const stop of stops) {
        ...
        if (previous && destination) {
          try {
            const response = await fetch('/api/route-time', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ origin: { latitude: previous[0], longitude: previous[1] }, destination: { latitude: destination[0], longitude: destination[1] } }) });
            if (response.ok) { const data = await response.json() as { durationMinutes: number }; travel = roundTravelMinutes(data.durationMinutes); }
          } catch { travel = 30; }
        }
        const arrivalTime = addMinutes(cursor, travel);
        next.push({ ...stop, arrivalTime });
        cursor = addMinutes(arrivalTime, stop.durationMinutes);
        previous = destination;
      }
      if (!cancelled) { setStops((current) => current.map((stop, index) => ({ ...stop, arrivalTime: next[index].arrivalTime }))); setRouteStatus('idle'); }
    };
    calculate();
    return () => { cancelled = true; };
  }, [routeKey]);
