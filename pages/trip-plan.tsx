...
export default function TripPlanPage() {
    ...
    const [startPoint, setStartPoint] = useState(villas[0].slug);
    const [endAtStart, setEndAtStart] = useState(true);
    const [endPoint, setEndPoint] = useState(villas[0].slug);
    const [customPointInput, setCustomPointInput] = useState('');
    const [endCustomPointInput, setEndCustomPointInput] = useState('');
    const [customStartForRoute, setCustomStartForRoute] = useState<CustomRoutePoint | null>(null);
    const [customEndForRoute, setCustomEndForRoute] = useState<CustomRoutePoint | null>(null);
    ...
    const customCoordinates = useCustomDayPoint(customPointInput, isCustomStart);
    const endCustomCoordinates = useCustomDayPoint(endCustomPointInput, isCustomEnd && !endAtStart);
    const startPointName = isAirportStart ? (locale === "uk" ? "\u0410\u0435\u0440\u043E\u043F\u043E\u0440\u0442 \u041C\u0430\u0434\u0435\u0439\u0440\u0438" : "Madeira Airport") : isCustomStart ? (customStartForRoute?.name ?? (locale === "uk" ? "\u0414\u043E\u0432\u0456\u043B\u044C\u043D\u0430 \u0442\u043E\u0447\u043A\u0430" : "Custom point")) : selectedVilla.name;
    ...
    const hasTrekking = routeLocations.some((location) => location?.tags.includes('Hiking') || location?.tags.includes('Outdoor activities'));
    ...
    useEffect(() => {
        ...
        const loadWeather = async () => {
            ...
            setWeather(null);
            try {
                const coordinates = slugs.map((slug) => locationCoordinates[slug] ?? [32.72, -16.97] as [
                    number,
                    number
                ]);
                ...
                if (hasBeach) {
                    ...
                    if (marine.ok) {
                        ...
                        const values = (data.hourly.sea_surface_temperature as Array<number | null>).filter((value): value is number => value !== null);
                        if (values.length)
                            waterTemperature = values.reduce((sum, value) => sum + value, 0) / values.length;
                    }
                }
                if (!cancelled) {
    ...
    const routeKey = JSON.stringify([startCoordinates, finalCoordinates, departureTime, stops.map(stop => [stop.id, stop.type, stop.slug, stop.latitude, stop.longitude, stop.durationMinutes, stop.boatDirection]), [selectedVilla.latitude, selectedVilla.longitude]]);
    ...
    const recommendationLines = useMemo(() => {
        const lines: string[] = [];
        const unavailable = weatherStatus === 'unavailable' ? text.weatherUnavailable : '';
        ...
        if (recommendations.fanal && hasFanal) {
            ...
            lines.push(locale === 'uk' ? 'Ліс розташований на висоті близько 1 100 м.' : 'The forest is at approximately 1,100 m above sea level.');
        ...
        if (recommendations.lastDay && hasLastDay) {
            ...
            lines.push(locale === 'uk' ? 'Це останній день програми. Рекомендую спакувати речі звечора, щоб уранці бути готовими до виселення. Через обмежений час організованого обіду цього дня не буде. За бажанням можна самостійно пообідати в Loft Brunch e Cocktails [na.zona velha] біля парковки.' : 'This is the final day of the programme. I recommend packing your bags the evening before so you are ready to check out in the morning. Time is limited, so there is no organised lunch today. For an optional independent lunch, consider Loft Brunch e Cocktails [na.zona velha] by the car park.');
            lines.push('https://maps.app.goo.gl/QW9bqQU3QPrirTej8', '');
        }
    ...
    const programme = useMemo(() => {
        ...
        stops.forEach((stop, index) => {
            ...
            if (index > 0) {
                ...
                const previousEndTime = addMinutes(previousStop.arrivalTime, previousStop.durationMinutes);
                const travelMinutes = minutesBetween(previousEndTime, stop.arrivalTime);
                lines.push(travelLine(previousEndTime, stopName(previousStop), travelMinutes), '');
            }
            if (stop.type === 'custom') {
                const wazeUrl = stop.latitude !== undefined && stop.longitude !== undefined ? `https://www.waze.com/ul?ll=${encodeURIComponent(`${stop.latitude},${stop.longitude}`)}&navigate=yes` : '';
                lines.push(`📍 ${stop.arrivalTime}–${endTime} — ${stop.name ?? (locale === 'uk' ? 'Інша локація' : 'Other location')}.`, ...(wazeUrl ? [`Waze: ${wazeUrl}`] : []), '');
                return;
            }
            if (stop.type === 'villa') {
            ...
            if (stop.type === 'location' && stop.slug === 'madeira-international-airport') {
                ...
                if (airport)
                    lines.push(stop.isLastDay ? `✈️ ${stop.arrivalTime} — ${locale === 'uk' ? 'Прибуття до Міжнародного аеропорту Мадейри для вильоту.' : 'Arrival at Madeira International Airport for departure.'}` : `✈️ ${stop.arrivalTime}–${endTime} — ${airport.name}.`, `https://madeiralivecams.com/${locale}/explore/${airport.slug}`, '');
                return;
            }
            const location = stop.slug ? locationBySlug.get(stop.slug) : undefined;
            if (!location)
                return;
            const icon = getRouteStopIcon(location);
            const sunriseSuffix = stop.slug === 'pico-do-arieiro' && stop.isSunrise ? locale === 'uk' ? ' Зустрічаємо схід сонця + прогулянка по маршруту PR1 – Vereda do Areeiro до Miradouro da Pedra Rija.' : ' Sunrise viewing plus a walk on PR1 – Vereda do Areeiro to Miradouro da Pedra Rija.' : '';
            const cristovaoSuffix = stop.slug === 'miradouro-sao-cristovao' && (stop.hasCristovaoBar || stop.hasCristovaoRestaurant)
                ...
                : '';
            const sunsetSuffix = stop.slug === 'pico-ruivo' && stop.isSunset ? locale === 'uk' ? ' Підйом від парковки Achada do Teixeira до Pico Ruivo (1 862 м) стежкою PR1.2, зустріч заходу сонця (~20 хв) та повернення тією самою стежкою до парковки.' : ' Hike PR1.2 from Achada do Teixeira car park to Pico Ruivo (1,862 m), watch the sunset (~20 min), then return along the same trail to the car park.' : '';
            const boatSuffix = stop.slug === 'madeira-sea-emotions' && stop.boatDirection === 'officeToCafe'
                ? (locale === 'uk' ? ' PR8 – Vereda da Ponta de São Lourenço + екскурсійний човен до маяка: офіс Madeira Sea Emotions → Casa de Sardinha, повернення пішки до офісу. Вартість екскурсії — 30 €.' : ' PR8 – Vereda da Ponta de São Lourenço + lighthouse boat excursion: Madeira Sea Emotions office → Casa de Sardinha, walk back to the office. Tour price: €30.')
            ...
            const routeInstruction = stop.slug === 'levada-nova-levada-do-moinho' && stop.showRouteInstructions
                ? (locale === 'uk' ? 'Обов’язково прочитайте інструкцію, як проходити маршрут ↓' : 'Please read the instructions for this route before setting out ↓')
                : '';
            lines.push(`${icon} ${stop.arrivalTime}–${endTime} — ${location.tags.includes("Restaurants") ? restaurantStopTitle(location.name, stop.arrivalTime, locale) : location.name}${boatSuffix.startsWith(' +') ? '' : '.'}${sunriseSuffix}${sunsetSuffix}${boatSuffix}${whaleSuffix}${cristovaoSuffix}`, ...(routeInstruction ? [routeInstruction] : []), `https://madeiralivecams.com/${locale}/explore/${location.slug}`, '');
        });
  ...
  + {'\u{1F4CD}'}
                ...
                return <article key={stop.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-ocean text-sm font-bold text-white">{index + 1}</span><div className="min-w-0 flex-1"><><p className="font-bold text-navy">{icon} {name}</p><RestaurantPlannerDetails slug={stop.slug} locale={locale}/></><p className="mt-1 text-xs text-slate-500">{durationLabel(stop.durationMinutes, locale)}</p></div><button type="button" onClick={() => removeStop(stop.id)} aria-label={text.remove} className="rounded-lg px-2 py-1 text-sm font-semibold text-slate-500 hover:bg-red-50 hover:text-red-600">×</button></div><div className="mt-4 grid grid-cols-2 gap-3"><label className="flex flex-col gap-1 text-xs font-semibold text-slate-600">{text.arrival}<input type="time" value={stop.arrivalTime} readOnly className="min-h-10 rounded-lg border border-slate-300 bg-slate-100 px-2 text-sm text-navy"/></label><label className="flex flex-col gap-1 text-xs font-semibold text-slate-600">{text.duration}<select value={stop.durationMinutes} onChange={(event) => updateStop(stop.id, { durationMinutes: Number(event.target.value) })} className="min-h-10 rounded-lg border border-slate-300 px-2 text-sm text-navy focus:border-ocean focus:outline-none">{durationOptions.map((minutes) => <option key={minutes} value={minutes}>{durationLabel(minutes, locale)}</option>)}</select></label></div>{canSelectSunrise && <label className="mt-4 flex min-h-11 cursor-pointer items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-navy"><input type="checkbox" checked={Boolean(stop.isSunrise)} onChange={(event) => updateStop(stop.id, { isSunrise: event.target.checked })} className="h-4 w-4 rounded border-slate-300 text-ocean focus:ring-ocean"/><span>🌅 {text.sunrise}</span></label>}
                    {stop.slug === 'pico-ruivo' && <label className="mt-4 flex min-h-11 cursor-pointer items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-navy"><input type="checkbox" checked={Boolean(stop.isSunset)} onChange={(event) => updateStop(stop.id, { isSunset: event.target.checked, durationMinutes: event.target.checked ? 120 : 90 })} className="h-4 w-4 rounded border-slate-300 text-ocean focus:ring-ocean"/>🌇 {locale === 'uk' ? 'Захід сонця' : 'Sunset'}</label>}
                    {stop.slug === 'levada-nova-levada-do-moinho' && <label className="mt-4 flex min-h-11 cursor-pointer items-center gap-3 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-navy"><input type="checkbox" checked={Boolean(stop.showRouteInstructions)} onChange={(event) => updateStop(stop.id, { showRouteInstructions: event.target.checked })} className="h-4 w-4 rounded border-slate-300 text-ocean focus:ring-ocean"/>📖 {locale === 'uk' ? 'Інструкція по маршруту' : 'Route instructions'}</label>}
  ...
  {routeUsageStatus === 'ready' && routeUsage && routeUsage.remaining <= 0 && (<span className="font-semibold text-red-600">
      {locale === 'uk'
