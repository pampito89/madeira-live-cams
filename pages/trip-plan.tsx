...
export default function TripPlanPage() {
  ...
  const programme = useMemo(() => {
    ...
    const selectedDate = new Date(`${date}T12:00:00`); const formattedDate = new Intl.DateTimeFormat(locale === 'uk' ? 'uk-UA' : 'en-GB', { day: 'numeric', month: 'long' }).format(selectedDate); const ukrainianWeekdays = ['неділю', 'понеділок', 'вівторок', 'середу', 'четвер', 'п’ятницю', 'суботу']; const englishWeekday = new Intl.DateTimeFormat('en-GB', { weekday: 'long' }).format(selectedDate); const heading = locale === 'uk' ? `Програма на ${ukrainianWeekdays[selectedDate.getDay()]}, ${formattedDate}` : `Programme for ${englishWeekday}, ${formattedDate}`; const lines = [heading, '', `🚌 ${departureTime} — ${text.departureFrom} ${selectedVilla.name}.`, '']; const endsAtAirport = stops[stops.length - 1]?.type === 'location' && stops[stops.length - 1]?.slug === 'madeira-international-airport';
