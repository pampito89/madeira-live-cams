...
const customRoutePoint = 'custom-route-point';
...
export default function TripPlanPage() {
  ...
  const pointForStop = (stop: PlanStop): [number, number] | null => {
    ...
    if (stop.type === 'custom' && stop.latitude !== undefined && stop.longitude !== undefined) return [stop.latitude, stop.longitude];
  ...
  const addLocation = () => { if (selectedSlug === customRoutePoint) { void addGoogleMapsPoint(); return; } if (!selectedSlug) return; const selectedLocation = locations.find((location) => location.slug === selectedSlug); setStops((current) => [...current, { id: `${selectedSlug}-${Date.now()}`, type: 'location', slug: selectedSlug, arrivalTime: getNextArrivalTime(), durationMinutes: selectedLocation?.tags.includes('Airport') ? 15 : standardLocationDurations[selectedSlug] ?? 90, isSunrise: false }]); setSelectedSlug(''); };
  const addGoogleMapsPoint = async () => { if (!googleMapsUrl || isResolvingGoogleMapsUrl) return; setIsResolvingGoogleMapsUrl(true); setGoogleMapsUrlError(''); try { const response = await fetch('/api/resolve-route-point', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: googleMapsUrl }) }); const data = (await response.json()) as { name?: string; latitude?: number; longitude?: number; error?: string }; if (!response.ok || !data.name || data.latitude === undefined || data.longitude === undefined) throw new Error(data.error || 'Unable to resolve the Google Maps link.'); setStops((current) => [...current, { id: `custom-${Date.now()}`, type: 'custom', latitude: data.latitude, longitude: data.longitude, name: data.name, sourceUrl: googleMapsUrl, arrivalTime: getNextArrivalTime(), durationMinutes: 60 }]); setGoogleMapsUrl(''); setSelectedSlug(''); } catch (error) { setGoogleMapsUrlError(error instanceof Error ? error.message : 'Unable to resolve the Google Maps link.'); } finally { setIsResolvingGoogleMapsUrl(false); } };
  ...
  const programme = useMemo(() => {
    ...
    stops.forEach((stop, index) => {
      ...
      } if (stop.type === 'custom') { const wazeUrl = stop.latitude !== undefined && stop.longitude !== undefined ? `https://www.waze.com/ul?ll=${encodeURIComponent(`${stop.latitude},${stop.longitude}`)}&navigate=yes` : ''; lines.push(`📍 ${stop.arrivalTime}–${endTime} — ${stop.name ?? (locale === 'uk' ? 'Інша локація' : 'Other location')}.`, ...(wazeUrl ? [`Waze: ${wazeUrl}`] : []), ''); return; } if (stop.type === 'villa') { lines.push(`🏡 ${stop.arrivalTime}–${endTime} — ${selectedVilla.name}.`, `https://madeiralivecams.com/${locale}/stays/${selectedVilla.slug}`, ''); return; } if (stop.type === 'location' && stop.slug === 'madeira-international-airport') { const airport = locationBySlug.get(stop.slug); if (airport) lines.push(`✈️ ${stop.arrivalTime}–${endTime} — ${airport.name}.`, `https://madeiralivecams.com/${locale}/explore/${airport.slug}`, ''); return; } const location = stop.slug ? locationBySlug.get(stop.slug) : undefined; if (!location) return; const icon = getRouteStopIcon(location); const sunriseSuffix = stop.slug === 'pico-do-arieiro' && stop.isSunrise ? locale === 'uk' ? ' Зустрічаємо схід сонця + прогулянка по маршруту PR1 – Vereda do Areeiro до Miradouro da Pedra Rija.' : ' Sunrise viewing plus a walk on PR1 – Vereda do Areeiro to Miradouro da Pedra Rija.' : '';
