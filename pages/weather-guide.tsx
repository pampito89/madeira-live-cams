...
const PICO_DO_ARIEIRO = {
    latitude: 32.7352,
    longitude: -16.928,
    cameraSourceUrl: 'https://www.netmadeira.com/webcams-madeira/pico-do-arieiro',
};
...
function windyCloudUrl(sunrise: string) {
    ...
    const offsetMinutes = match
        ...
        : 0;
    const utcTime = new Date(Date.UTC(year, month - 1, day, hour, minute) - offsetMinutes * 60000);
...
export default function WeatherGuidePage() {
        ...
        const loadForecast = async () => {
            try {
                ...
                const days: SunriseDay[] = data.daily.time
                    ...
                    const exactSunriseIndex = data.hourly.time.findIndex((time: string) => time === sunriseHour);
                    const safeIndex = exactSunriseIndex >= 0 ? exactSunriseIndex : index * 24 + 7;
                    return {
                        date,
                        sunrise,
                        ...
                        rainChance: Math.round(data.hourly.precipitation_probability[safeIndex] ?? 0),
                        windSpeed: Math.round(data.hourly.wind_speed_10m[safeIndex] ?? 0),
                        windDirection: Math.round(data.hourly.wind_direction_10m[safeIndex] ?? 0),
                    };
                });
                setForecast({
                    loading: false,
    ...
    return (<Layout>
      <Head>
        ...
        <meta name="description" content={messages.weatherGuide.pageDescription}/>

        

        <meta property="og:title" content={messages.weatherGuide.pageTitle}/>

        <meta property="og:description" content={messages.weatherGuide.ogDescription}/>
      ...
      <main className="page-shell">
        <section className="relative isolate mb-6 overflow-hidden rounded-2xl bg-forest px-5 py-7 text-white shadow-lg shadow-forest/15 sm:px-8 sm:py-9">
          <div aria-hidden="true" className="absolute inset-0 -z-20 bg-cover bg-center md:hidden" style={{
            backgroundImage: "url('/images/pico-radar-mobile.png')",
        }}/>

          <div aria-hidden="true" className="absolute inset-0 -z-20 hidden bg-cover bg-center md:block" style={{
            backgroundImage: "url('/images/pico-radar-desktop.png')",
        }}/>

          <div aria-hidden="true" className="absolute inset-0 -z-10 bg-gradient-to-br from-[#092b22]/95 via-[#0b3d31]/82 to-[#11223b]/78"/>
        ...
        <section className="rounded-2xl border border-moss/60 bg-white p-3 shadow-sm sm:p-4">
          {forecast.loading && (<div className="rounded-xl border border-moss/40 bg-panel p-4 text-sm text-slate-600">
              {messages.weatherGuide.loading}
            </div>)}

          {forecast.error && (<div className="rounded-xl border border-lava/40 bg-lava/10 p-4 text-sm text-[#7A3021]">
              {forecast.error} {messages.weatherGuide.errorRetry}
            </div>)}

          {!forecast.loading && !forecast.error && (<div className="grid gap-2.5 lg:grid-cols-2">
              {forecast.days.map((day) => {
                const rating = sunriseRating(day);
                return (<article key={day.date} className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition hover:border-moss hover:shadow-md">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className={`flex shrink-0 items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-semibold ${rating.className}`} aria-label={`${rating.title}: ${rating.description}`} title={`${rating.title}: ${rating.description}`}>
                    ...
                    <div className="mt-2 grid grid-cols-3 overflow-hidden rounded-lg border border-moss/30 bg-panel text-center">
                      <div className="border-r border-moss/30 px-1.5 py-1.5" title={messages.weatherGuide.lowClouds}>
                      ...
                      <div className="border-r border-moss/30 px-1.5 py-1.5" title={messages.weatherGuide.midClouds}>
                        <p className="text-sm leading-4" aria-hidden="true">
                          ☁️
                        </p>
                      ...
                      <div className="px-1.5 py-1.5" title={messages.weatherGuide.highClouds}>
                        <p className="text-sm leading-4" aria-hidden="true">
                          ☁️⬆️
                        </p>
                    ...
                    <div className="mt-1.5 grid grid-cols-2 overflow-hidden rounded-lg border border-moss/30 bg-panel">
                      <div className="flex items-center justify-center gap-1 border-r border-moss/30 px-2 py-1.5" title={messages.weatherGuide.rainChance}>
                      ...
                      <div className="flex items-center justify-center gap-1 px-2 py-1.5" title={messages.weatherGuide.windAtSunrise}>
                        <span className="text-sm leading-4" aria-hidden="true">
                          💨
                        </span>
                        ...
                        <span className="inline-flex h-4 w-4 items-center justify-center text-base font-bold leading-none text-ocean" style={{
                        transform: `rotate(${day.windDirection}deg)`,
                    }} aria-label={`${day.windDirection}°`} title={`${day.windDirection}°`}>
                          ↑
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 flex gap-1.5">
                      <a href={PICO_DO_ARIEIRO.cameraSourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex flex-1 items-center justify-center rounded-md border border-ocean bg-white px-2 py-1.5 text-xs font-semibold text-ocean transition hover:bg-ocean hover:text-white focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2">
                        {cameraLabel}
                      </a>

                      <a href={windyCloudUrl(day.sunrise)} target="_blank" rel="noopener noreferrer" className="inline-flex flex-1 items-center justify-center rounded-md bg-ocean px-2 py-1.5 text-xs font-semibold text-white transition hover:bg-forest focus:outline-none focus:ring-2 focus:ring-leaf focus:ring-offset-2">
                        Windy
                      </a>
                    </div>
                  </article>);
          ...
          <section className="mt-4 rounded-lg border border-moss/30 bg-mist px-3 py-3 text-xs leading-5 text-slate-600">
            ...
            <p className="mt-2 text-slate-500">
              <span className="mr-1 inline-flex h-4 w-4 items-center justify-center text-base font-bold leading-none text-ocean" aria-hidden="true">
                ↑
              </span>
              {legend.arrow}
            </p>
