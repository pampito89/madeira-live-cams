...
test('all 31 venues have unique cards and valid Madeira coordinates', () => {
  assert.equal(restaurants.length, 31);
  assert.equal(new Set(restaurants.map(r => r.slug)).size, 31);
  assert.equal(new Set(restaurants.map(r => r.googleMapsUrl)).size, 31);
  ...
  assert.equal(restaurants.find(r => r.slug === 'restaurant-loft-brunch-zona-velha').googleMapsUrl, 'https://maps.app.goo.gl/QW9bqQU3QPrirTej8');
});
...
test('restaurants are discoverable in the planner, but not the public locations collection', () => {
  ...
  assert.equal(loadTs('../data/plannerLocations.ts').locations.filter(r => r.tags.includes('Restaurants')).length, 31);
