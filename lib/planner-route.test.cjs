...
test('a PR8 boat trip arrives at the trailhead and leaves by road from the boat office', async () => {
  const trailhead = [32.7443, -16.6994], office = [32.7416264, -16.7104624];
  const calls = [];
  const result = await calculatePlannerRoute({ ...options,
    stops: [{ ...stop, slug: 'ponta-de-sao-lourenco', durationMinutes: 150 }],
    pointForStop: async () => trailhead, pointAfterStop: async () => office,
    request: async (_url, config) => { calls.push(JSON.parse(config.body)); return reply(20); },
  });
  assert.equal(calls.length, 2);
  assert.deepEqual(calls[0].destination, { latitude: trailhead[0], longitude: trailhead[1] });
  assert.deepEqual(calls[1].origin, { latitude: office[0], longitude: office[1] });
  assert.equal(result.stops[0].arrivalTime, '09:20');
  assert.equal(result.arrival, '12:10');
});
