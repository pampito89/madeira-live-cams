...
type Stop = { id: string; type: 'location' | 'custom' | 'villa'; slug?: string; arrivalTime: string; durationMinutes: number };
type Draft = { stops: Stop[]; departureTime: string };
...
export function usePlannerDraft<T extends Draft>(draft: T, apply: (draft: Partial<T>) => void) {
  ...
  useEffect(() => {
    if (!router.isReady || booted.current) return;
    booted.current = true;
    ...
    try {
      ...
      if (saved?.version === 1 && saved.draft && typeof saved.draft === 'object') {
        for (const key of Object.keys(draft) as (keyof T)[]) {
          ...
          if (key === 'stops') {
            if (Array.isArray(value) && value.length <= 100 && value.every((stop: Stop) =>
              stop && typeof stop.id === 'string' && ['location', 'custom', 'villa'].includes(stop.type)
              && validTime(stop.arrivalTime) && Number.isFinite(stop.durationMinutes)
              && stop.durationMinutes >= 0 && stop.durationMinutes <= 1440)) restored[key] = value as T[typeof key];
          } else if (key === 'departureTime') {
            if (validTime(value)) restored[key] = value as T[typeof key];
            ...
            if (value === null || value && typeof value.name === 'string'
              ...
              && Number.isFinite(value.longitude) && Math.abs(value.longitude) <= 180) restored[key] = value;
          } else if (key === 'recommendations') {
            if (value && ['weather', 'beach', 'levada', 'sunrise', 'food'].every(option => typeof value[option] === 'boolean')) restored[key] = value;
    ...
    const requested = typeof router.query.addRestaurant === 'string' ? getRestaurant(router.query.addRestaurant) : undefined;
    if (requested) restored = { ...restored, stops: [...restored.stops, {
      id: `${requested.slug}-${Date.now()}`, type: 'location', slug: requested.slug,
      arrivalTime: nextArrival(restored), durationMinutes: 90,
    }] };
    apply(restored);
    setReady(true);
    if ('addRestaurant' in router.query) {
  ...
  useEffect(() => {
    if (!ready) return;
    try { sessionStorage.setItem(storageKey, JSON.stringify({ version: 1, draft })); }
