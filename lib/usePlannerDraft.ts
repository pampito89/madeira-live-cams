            ...
            if (saved?.version === 1 && saved.draft && typeof saved.draft === 'object') {
                for (const key of Object.keys(draft) as (keyof T)[]) {
                    ...
                    if (key === 'stops') {
                        ...
                        if (value && ['weather', 'beach', 'levada', 'sunrise', 'food'].every(option => typeof value[option] === 'boolean') && ['fanal', 'lastDay', 'trekking'].every(option => value[option] === undefined || typeof value[option] === 'boolean'))
                            restored[key] = value;
                    }
